...
CHANGELOG:
  1.0.0 - Initial version. Read-only candles.db access, build_levels/analyze_day port
          of the JS engine, asia_sweep/ict_fvg/realized_outcome/smt_confluence
          aggregation. Depended on an external gametrader_stats_schema.sql file.
  1.1.0 - Made fully self-contained: schema embedded as SCHEMA_SQL string constant,
          no longer reads a sibling .sql file. Removed now-unused Path import.
  1.1.1 - Fixed a real bug found against production data: aggregate_realized_outcome()
          counted every retested day toward the denominator even when analyze_day's
          degenerate stop_dist<=0 guard had left that day's outcomes unpopulated,
          which broke the hit_target+hit_stop+no_hit==denominator invariant. Fixed by
          computing each target_type's denominator from records that actually have a
          computed outcome for it, rather than from retest_ix presence alone.
  1.1.2 - Fixed a real bug found via production sanity-check: ict_fvg.retested was
          reporting 100.00% across ~400 real samples on both asiaHi and asiaLo, which
          is a mathematical impossibility, not a real market pattern. Root cause: the
          retest scan started checking AT the FVG-forming bar itself, whose own
          high/low were literally used to define the zone boundary -- a tautology
          that always passed on the first bar checked. Fixed by starting the scan at
          fvg["ix"]+1. Verified genuine retests (both on the immediately next bar and
          several bars later) still detect correctly, and confirmed the live JS engine
          never had this bug (it only checks ticks strictly after the phase
          transition, never the same tick that formed the FVG).
...
also embedded below as SCHEMA_SQL).
...
SCHEMA_SQL = """
-- gametrader_stats.db  (embedded schema version: 2.0.0 -- keep in sync with
-- gametrader_stats_schema.sql's changelog if this ever changes)
...
def analyze_day(level: Level, side: int) -> DayAnalysis:
    ...
    # IMPORTANT: must start at fvg["ix"]+1, not fvg["ix"] itself. The FVG-forming bar's
    # own high/low were literally used to define the zone boundary (hi = that bar's own
    # low, for a bullish FVG), so checking that same bar against its own zone is a
    # tautology that always passes -- it isn't a genuine "price came back" event. This
    # was a real bug (found via production sanity-check: retested was 100.00% across
    # ~400 samples, which is a mathematical certainty of a bug, not a real pattern).
    retest_end = min(len(bars) - 1, fvg["ix"] + FVG_RETEST_BARS)
    retest_ix = None
    for k in range(fvg["ix"] + 1, retest_end + 1):
...
def run(stats_db_path: str = STATS_DB_PATH) -> None:
    print(f"compute_gametrader_stats.py v{__version__}")
